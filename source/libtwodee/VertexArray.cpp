...
VertexArray::VertexArray(const ShaderProgram& program,
  ...
  for (int i = 0; i < attributes.GetAttributeCount(); ++i) {
    ...
    GLint loc = glGetAttribLocation(program.GetID(), attribute->name.c_str());
    if (loc < 0) {
      std::cout << "Attribute " << attribute->name << " is not referenced in the shaders." << std::endl;
    } else {
  ...
  for (int i = 0; i < attributes.GetAttributeCount(); ++i) {
    const VertexAttribute *attribute = attributes.GetAttribute(i);
    locations[i] = glGetAttribLocation(program.GetID(), attribute->name.c_str());
    if (locations[i] < 0) {
      std::cout << "Attribute " << attribute->name << " is not referenced in the shaders." << std::endl;
    } else {
      glBindBuffer(GL_ARRAY_BUFFER, attribute->bid);
